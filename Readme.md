...
WORKDIR /src
COPY ["Poc-LR.csproj", "Poc-LR/"]
RUN dotnet restore "Poc-LR/Poc-LR.csproj"

WORKDIR "/src/Poc-LR"
COPY . .
RUN dotnet build "Poc-LR.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "Poc-LR.csproj" \
...
Execute unit tests
```
    cd "Poc-LR/test/Poc-LR.Tests"
    dotnet test
```

Deploy application
```
    cd "Poc-LR/src/Poc-LR"
